...
switch(process.env.platform) {
    case 'linux': 
    case 'macos':
        module.exports = function(statEmitter, diskfilesystems, mounts, threshold) {
            ...
            exec(df, {stdio:['inherit','pipe','pipe']}, function(err, output, code) {
                ...
                drives.forEach(function(drive) {
                    ...
                    if(diskfilesystems && diskfilesystems.indexOf(filesystem) < 0) return;
                    ...
                    if(mounts && mounts.indexOf(mount) < 0) return;
                    let usedpct = Number(parseFloat((total - free) / total * 100).toFixed(2));
                    
                    if(!threshold || threshold === 0 || usedpct > threshold) {
